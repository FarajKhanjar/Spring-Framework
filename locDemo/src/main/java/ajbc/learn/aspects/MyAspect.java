...
public class MyAspect {
	...
	// "execution(modifier ? return-type? method-pattern)"

	@Before("execution(* ajbc.learn.dao.ProductDao.*(..))")  //joinPoint just get info!
	public void logBeforeCalling(JoinPoint joinPoint) {
		System.out.println("Aspect is writing to logger signature: "+joinPoint.getSignature().getName());
		System.out.println("args are "+Arrays.toString(joinPoint.getArgs()));
	}
	
	//the solution of max->min range
